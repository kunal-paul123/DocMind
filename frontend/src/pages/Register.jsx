import { Link } from 'react-router-dom';
import './Auth.css';

const GOOGLE_AUTH_URL = 'http://localhost:8000/auth/google';

export default function Register() {
  const handleGoogleSignup = () => {
    window.location.href = GOOGLE_AUTH_URL;
  };

  return (
    <div className="auth-page">
      <div className="auth-bg">
        <div className="auth-orb auth-orb-1" />
        <div className="auth-orb auth-orb-2" />
      </div>

      <Link to="/" className="auth-logo">
        <span className="logo-icon">⬡</span>
        <span className="logo-text">DocMind</span>
      </Link>

      <div className="auth-card fade-in">
        <div className="auth-card-header">
          <div className="auth-icon-wrap">
            <span className="auth-icon">✦</span>
          </div>
          <h1 className="auth-title">Create an account</h1>
          <p className="auth-subtitle">Start chatting with your documents today</p>
        </div>

        {/* Benefits */}
        <ul className="auth-benefits">
          {['Upload unlimited PDFs', 'AI-powered semantic search', 'Source citations per answer', 'Streaming real-time responses'].map(b => (
            <li key={b} className="auth-benefit-item">
              <span className="benefit-check">✓</span>
              {b}
            </li>
          ))}
        </ul>

        <button
          id="google-signup-btn"
          className="btn btn-google"
          onClick={handleGoogleSignup}
        >
          <GoogleIcon />
          Sign up with Google
        </button>

        <div className="auth-footer-text" style={{ marginTop: 20 }}>
          Already have an account?{' '}
          <Link to="/login" className="auth-link">Sign in</Link>
        </div>

        <p className="auth-terms">
          By signing up, you agree to our{' '}
          <a href="#" className="auth-link">Terms of Service</a> and{' '}
          <a href="#" className="auth-link">Privacy Policy</a>.
        </p>
      </div>
    </div>
  );
}

function GoogleIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24">
      <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
      <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
      <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
      <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
    </svg>
  );
}
